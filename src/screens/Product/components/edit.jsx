...
const Edit = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      if (response.success === "true") {
        ...
        // Set preview images to display existing images
        ...
        setFormData({
          ...
          color: product.color || "#FFFFFF", // Provide a default color if none is provided
          size: product.size,
          ...
          categories: product.categories.map((category) => category._id), // Just use the category IDs for the selected value
          images: product.images, // Store image paths if needed for submission
        });
      } else {
        notification.error({
          message: "Error",
  ...
  // On component mount, fetch categories and product data
  useEffect(() => {
    fetchCategories(); // Fetch categories
    if (id) {
      fetchProductData(); // Fetch product data if ID is provided
    }
  }, [id]);
  ...
  const handleCategoryChange = (selectedOptions) => {
    // Log selected options to check their structure
    console.log("=======selectedOptions", selectedOptions);

    // Map selected options to only get the 'value' field for storage
    const selectedIds = selectedOptions?.map((option) => option.value) ?? [];

    // Update the form data with the selected category IDs
    handleSelectChange("categories", selectedIds);
  };
  ...
  return (
    <>
      {loadingCategories && <LoaderOverlay />}
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-normal text-xl leading-6">Update Product</h1>
              ...
              <Row gutter={[20, 20]}>
                ...
                <Col xs={24} sm={12} md={8} lg={8}>
                  <Select
                    isMulti
                    className="bg-[#000000]"
                    options={categories} // Ensure this contains the correctly formatted categories
                    value={categories
                      .filter(
                        (cat) => formData.categories.includes(cat.value) // Ensure we're checking the correct value
                      )
                ...
                <Col xs={24} sm={12} md={8} lg={8}>
                  <div className="card-body">
                    <div className="dropzone needsclick p-0 border-2 border-dotted border-gray-300 rounded-lg p-5 text-center">
                      ...
                      <p className="h6 text-muted d-block font-normal mb-2">
                        or
                      </p>
                      <span
